...
RSASignature::generate_key_pair(size_t bits)
{
  ...
  if (EVP_PKEY_CTX_set_rsa_keygen_bits(ctx.get(), static_cast<int>(bits)) <= 0) {
    throw openssl_error();
  }
...
RSASignature::deserialize(const bytes& enc) const
{
  ...
  auto* pkey = d2i_PublicKey(EVP_PKEY_RSA, nullptr, &data_ptr, static_cast<int>(enc.size()));
  if (pkey == nullptr) {
    throw openssl_error();
  }
...
RSASignature::deserialize_private(const bytes& skm) const
{
  ...
  auto* pkey = d2i_PrivateKey(EVP_PKEY_RSA, nullptr, &data_ptr, static_cast<int>(skm.size()));
  if (pkey == nullptr) {
    throw openssl_error();
  }
