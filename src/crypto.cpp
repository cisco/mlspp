...
SignaturePrivateKey
SignaturePrivateKey::from_external_callback(
  CipherSuite suite,
                          ...
                          const bytes& message) const
{
  ...
  if (external_key_) {
    // Use external key for signing
    return suite.sig().sign_external(content, *external_key_);
  }
...
bool
SignaturePrivateKey::exportable() const
{
  if (external_key_) {
