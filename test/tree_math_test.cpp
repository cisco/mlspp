...
const std::vector<size_t> root{ 0x00, 0x01, 0x03, 0x03, 0x07, 0x07,
                                0x07, 0x07, 0x0f, 0x0f, 0x0f };
const std::vector<size_t> index{ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
                                 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d,
                                 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14 };

const std::vector<size_t> left{ 0x00, 0x00, 0x02, 0x01, 0x04, 0x04, 0x06,
                                0x03, 0x08, 0x08, 0x0a, 0x09, 0x0c, 0x0c,
                                0x0e, 0x07, 0x10, 0x10, 0x12, 0x11, 0x14 };

const std::vector<size_t> right{ 0x00, 0x02, 0x02, 0x05, 0x04, 0x06, 0x06,
                                 0x0b, 0x08, 0x0a, 0x0a, 0x0d, 0x0c, 0x0e,
                                 0x0e, 0x13, 0x10, 0x12, 0x12, 0x14, 0x14 };

const std::vector<size_t> parent{ 0x01, 0x03, 0x01, 0x07, 0x05, 0x03, 0x05,
                                  0x0f, 0x09, 0x0b, 0x09, 0x07, 0x0d, 0x0b,
                                  0x0d, 0x0f, 0x11, 0x13, 0x11, 0x0f, 0x13 };

const std::vector<size_t> sibling{ 0x02, 0x05, 0x00, 0x0b, 0x06, 0x01, 0x04,
                                   0x13, 0x0a, 0x0d, 0x08, 0x03, 0x0e, 0x09,
                                   0x0c, 0x0f, 0x12, 0x14, 0x10, 0x07, 0x11 };

const std::vector<std::vector<size_t>> frontier{
  { 0x00 },       { 0x01 },       { 0x01, 0x04 },       { 0x03 },
  { 0x03, 0x08 }, { 0x03, 0x09 }, { 0x03, 0x09, 0x0c }, { 0x07 },
  { 0x07, 0x10 }, { 0x07, 0x11 }, { 0x07, 0x11, 0x14 },
};

const std::vector<std::vector<size_t>> dirpath{
  { 0x07, 0x03, 0x01 },
  { 0x07, 0x03 },
  { 0x07, 0x03, 0x01 },
  { 0x07 },
...
const std::vector<std::vector<size_t>> copath{
  { 0x13, 0x0b, 0x05, 0x02 },
  { 0x13, 0x0b, 0x05 },
  { 0x13, 0x0b, 0x05, 0x00 },
  { 0x13, 0x0b },
...
const std::vector<size_t> leaves{ 0x00, 0x02, 0x04, 0x06, 0x08, 0x0a,
...
{
  for (const size_t& i : answers::index) {
...
{
  return [=](size_t x) -> auto { return function(x, answers::n); };
}
...
TEST_CASE("Tree math functions match expected outputs", "[tree-math]")
{
  SECTION("root")
  {
    for (size_t n = 1; n < answers::n; n += 1) {
