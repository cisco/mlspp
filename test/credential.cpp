...
int generate_set_random_serial(X509 *crt)
{
  ...
  serial_bytes[0] &=  static_cast<unsigned  int>(0x7f); /* Ensure positive serial! */
  BIGNUM *bn = BN_new();
...
std::tuple<EVP_PKEY*, EVP_PKEY*> newEd25519SigningKeyPair() {
    ...
    auto* pkey = EVP_PKEY_new_raw_private_key(EVP_PKEY_ED25519, nullptr, data.data(), data.size());
    ...
    auto* pubKey =
            EVP_PKEY_new_raw_public_key(EVP_PKEY_ED25519, nullptr, raw.data(), raw.size());
    if (pubKey == nullptr) {
      throw openssl_error();
    }
...
static X509* make_cert(CertTemplate cert_template, EVP_PKEY* public_key, EVP_PKEY* priv_key ) {
    X509 *cert = nullptr;
    X509V3_CTX ctx;
  ...
  if ((name = X509_NAME_new()) == nullptr ||
      X509_NAME_add_entry_by_NID(
              ...
              reinterpret_cast<unsigned char*>(common.data()), -1, -1, 0)  == 0 ||
    ...
    X509_gmtime_adj(X509_get_notAfter(cert),static_cast<int64_t>(60*60*24*days));
